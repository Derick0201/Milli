...
bool assertion_thrown = false;
#define assert(x) if(!(x)) assertion_thrown=true;
...
BOOST_AUTO_TEST_SUITE(not_empty_test_suite)
  ...
  BOOST_AUTO_TEST_CASE(no_default_constructor) {
    constexpr bool is_default_constructible = std::is_default_constructible<not_empty<int*>>::value;
    BOOST_TEST(is_default_constructible == false);
  }

  BOOST_AUTO_TEST_CASE(no_nullptr_t_constructor) {
    constexpr bool is_nullptr_constructible = std::is_constructible<not_empty<int*>, std::nullptr_t>::value;
    constexpr bool is_nullptr_constructible_ref = std::is_constructible<not_empty<int*>, std::nullptr_t&>::value;
    constexpr bool is_nullptr_constructible_rref = std::is_constructible<not_empty<int*>, std::nullptr_t&&>::value;
    constexpr bool is_nullptr_constructible_cref = std::is_constructible<not_empty<int*>, const std::nullptr_t&>::value;
    constexpr bool is_nullptr_constructible_crref = std::is_constructible<not_empty<int*>, const std::nullptr_t&&>::value;
  ...
  BOOST_AUTO_TEST_CASE(no_nullopt_t_constructor) {
    constexpr bool is_nullopt_constructible = std::is_constructible<not_empty<int*>, std::nullopt_t>::value;
    constexpr bool is_nullopt_constructible_ref = std::is_constructible<not_empty<int*>, std::nullopt_t&>::value;
    constexpr bool is_nullopt_constructible_rref = std::is_constructible<not_empty<int*>, std::nullopt_t&&>::value;
    constexpr bool is_nullopt_constructible_cref = std::is_constructible<not_empty<int*>, const std::nullopt_t&>::value;
    constexpr bool is_nullopt_constructible_crref = std::is_constructible<not_empty<int*>, const std::nullopt_t&&>::value;
  ...
  BOOST_AUTO_TEST_CASE(null_pointer_initialization_fails) {
    int* ptr = nullptr;
    not_empty<int*> test(ptr);
    BOOST_TEST(assertion_thrown);
    assertion_thrown = false;
  }

  BOOST_AUTO_TEST_CASE(constructor_test) {
    int test_value;
    not_empty<int*> intptr(&test_value);
    *intptr = 1;

    BOOST_TEST(not assertion_thrown);
  ...
  BOOST_AUTO_TEST_CASE(smart_ptr_empty_test) {
    ...
    not_empty<std::unique_ptr<int>> not_null_ptr(std::move(my_test));
  ...
  BOOST_AUTO_TEST_CASE(optional_empty_type_test) {
    ...
    not_empty<std::optional<int>> not_null_optional(empty_optional);
  ...
  BOOST_AUTO_TEST_CASE(deduction_guide) {
    not_empty value(new int);

    constexpr bool deduction_type_check = std::is_same<decltype(value), not_empty<int*>>::value;
  ...
  BOOST_AUTO_TEST_CASE(assignment_operator) {
    ...
    test = not_empty<int*>(&first);
    BOOST_TEST(*test == first);

    constexpr bool noexcept_not_empty_pointer = noexcept(test = not_empty<int*>(&first));
    BOOST_TEST(noexcept_not_empty_pointer);

    auto shared_val_1 = std::make_shared<int>(1);
    auto shared_val_2 = std::make_shared<int>(2);
    not_empty<std::shared_ptr<int>> shared_test = shared_val_1;
    ...
    not_empty<A> throwable_test(A{});
    throwable_test = not_empty<A>(A{});
    constexpr bool noexcept_false = not noexcept(throwable_test = not_empty<A>(A{}));
    BOOST_TEST(noexcept_false);

    constexpr bool noexcept_false_2 = not noexcept(throwable_test = A{});
    BOOST_TEST(noexcept_false_2);
  ...
  BOOST_AUTO_TEST_CASE(no_nullptr_assignment) {
    constexpr bool is_nullptr_assignable = std::is_assignable<not_empty<int*>, std::nullptr_t>::value;
    constexpr bool is_nullptr_assignable_ref = std::is_assignable<not_empty<int*>, std::nullptr_t&>::value;
    constexpr bool is_nullptr_assignable_rref = std::is_assignable<not_empty<int*>, std::nullptr_t&&>::value;
    constexpr bool is_nullptr_assignable_cref = std::is_assignable<not_empty<int*>, const std::nullptr_t&>::value;
    constexpr bool is_nullptr_assignable_crref = std::is_assignable<not_empty<int*>, const std::nullptr_t&&>::value;
  ...
  BOOST_AUTO_TEST_CASE(no_nullopt_assignment) {
    constexpr bool is_nullopt_assignable = std::is_assignable<not_empty<int*>, std::nullopt_t>::value;
    constexpr bool is_nullopt_assignable_ref = std::is_assignable<not_empty<int*>, std::nullopt_t&>::value;
    constexpr bool is_nullopt_assignable_rref = std::is_assignable<not_empty<int*>, std::nullopt_t&&>::value;
    constexpr bool is_nullopt_assignable_cref = std::is_assignable<not_empty<int*>, const std::nullopt_t&>::value;
    constexpr bool is_nullopt_assignable_crref = std::is_assignable<not_empty<int*>, const std::nullopt_t&&>::value;
  ...
  BOOST_AUTO_TEST_CASE(constructor_accepts_pointer_of_child_type) {
    ...
    not_empty<A*> parent_ptr(&test_parent);
    not_empty<B*> child_ptr(&test_child);

    not_empty<A*> result_ptr(child_ptr);

    BOOST_TEST(parent_ptr != child_ptr);
    BOOST_TEST(result_ptr == child_ptr);

    not_empty<A*> direct_initialized(&test_child);
  ...
  BOOST_AUTO_TEST_CASE(cast_operator) {
    ...
    not_empty<std::shared_ptr<int>> not_empty(ptr);
  ...
  BOOST_AUTO_TEST_CASE(dereference_operator) {
    ...
    const not_empty<int*>& const_test = test;
  ...
  BOOST_AUTO_TEST_CASE(casting_operator){
    ...
    const not_empty<int*>& const_test_ne = test_ne;
  ...
  BOOST_AUTO_TEST_CASE(member_types) {
    using tested_type = not_empty<int*>;
    constexpr bool simple_pointer_element_type = std::is_same<tested_type::element_type, int>::value;
    constexpr bool simple_pointer_stored_type = std::is_same<not_empty<int*>::stored_type, int*>::value;
  ...
  BOOST_AUTO_TEST_CASE(arrow_dereference) {
    ...
    not_empty<std::shared_ptr<Foo>> test(std::shared_ptr<Foo>(new Foo));
